...
if (dateAsGlobal === getSubtractedDate(0)) window.location.href = "index.html"
...
async function setContent(dataList, append = false) {
  ...
  dataList.forEach(r => {
    ...
    const percentages = transformToPercentages(r.new_isTrue, r.new_isFalse, r.new_isUnclear)
    ...
    const getColorOverlay = () => {
      if (isTruePerc > isFalsePerc  && isTruePerc > isUnclearPerc) return "bg-green-500";
      else if (isFalsePerc > isTruePerc  && isFalsePerc > isUnclearPerc) return "bg-red-500";
      else if (isUnclearPerc > isFalsePerc && isUnclearPerc > isTruePerc ) return "bg-orange-500";
      else return "";
    }
    ...
    news_div.innerHTML += /* html */
      `<div id="${r.new_id}" class="mb-5">
                <div>
                  ...
                  <button title="Open this new chat" onclick="readOnlyChat('${r.new_id}', '${r.new_title}')" class="mt-5 bg-[#2c2c2c] hover:bg-[#7b7575] text-[8pt] text-white font-bold py-1 px-2 rounded inline-flex items-center">
                  ...
                  <p class="mt-4 text-neutral-500 dark:text-neutral-500">
                    <small>${dateAsGlobal} | <u>${r.new_source}</u></small>
                  </p>
                </div>
              </div>`
  })
