...
class CnnPortugalNewsCollector(scrapy.Spider):
	...
	def parse(self, response):
		...
		for x in range(len(news_title)):
			print(f"index is {x}")
			data.append({"new_id":str(uuid.uuid4()), "new_link": news_link[x], "new_title": news_title[x], "new_desc": "", "new_date": news_date[x], "new_img": news_img[x], "new_source": "cnnPortugal", "new_isTrue": 0, "new_isFalse": 0, "new_isUnclear": 0, "new_votedIps": []})
