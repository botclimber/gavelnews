...
async function setContent(dataList) {
  ...
  dataList.forEach(r => {
    ...
    const totalVotes = (r.new_isTrue) + (r.new_isFalse) + (r.new_isUnclear) + (r.new_noOpinion)

    const isTruePerc = computeAvg(r.new_isTrue, totalVotes)
...
async function sortBy(veracityValue) {
  ...
  manipulatedData = manipulatedData.sort((a, b) => b[veracityValue] - a[veracityValue]);
